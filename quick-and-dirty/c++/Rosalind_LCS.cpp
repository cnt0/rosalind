...
{
    REP(i, 256)
    if (tree[v].links[i].end > 0)
        /*{
            debug(tree[v].links[i].start);
            debug(tree[v].links[i].end);
        }*/
...
{
#ifndef ONLINE_JUDGE
    freopen("rosalind_lcs.txt", "r", stdin); freopen("output.txt", "w", stdout);
#endif
    size_t len = 0;
    sample = "";
    string line;
