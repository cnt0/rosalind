...
{
#ifndef ONLINE_JUDGE
    freopen("rosalind_revp.txt", "r", stdin); freopen("output.txt", "w", stdout);
#endif
    string dna; cin >> dna;
    int n = dna.length();   
    REP(i, n - 3) REP2(j, 4, min(9, n - i + 1))
    {
        string target = dna.substr(i, j);
        if (target == reverse_complement(target)) cout << (i + 1) << ' ' << j << '\n';
    }
    return 0;
}